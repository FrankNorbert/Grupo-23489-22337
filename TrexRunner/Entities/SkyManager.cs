    ...
    public class SkyManager : IGameEntity, IDayNightCycle
    {
        ...
        public void Draw(SpriteBatch spriteBatch, GameTime gameTime)
        {
            if(OverlayVisibility > EPSILON)
                spriteBatch.Draw(_overlay, new Rectangle(0, 0, Main.WINDOW_WIDTH, Main.WINDOW_HEIGHT), Color.White * OverlayVisibility);
        ...
        public void Update(GameTime gameTime)
        {
            if(_moon == null)
            {
                _moon = new Lua(this, _spriteSheet, _trex, new Vector2(Main.WINDOW_WIDTH, MOON_POS_Y));
            ...
            {
                if(skyObject is Lua moon)
                {
                    moon.Position = new Vector2(Main.WINDOW_WIDTH, MOON_POS_Y);
                }
                else
        ...
        private void HandleCloudSpawning()
        {
            ...
            if (clouds.Count() <= 0 || (Main.WINDOW_WIDTH - clouds.Max(c => c.Position.X)) >= _targetCloudDistance)
            {
                ...
                Cloud cloud = new Cloud(_spriteSheet, _trex, new Vector2(Main.WINDOW_WIDTH, posY));
        ...
        {
            IEnumerable<Estrela> stars = _entityManager.GetEntitiesOfType<Estrela>();

            if(stars.Count() <= 0 || (Main.WINDOW_WIDTH - stars.Max(c => c.Position.X)) >= _targetStarDistance)
            {
                ...
                Estrela star = new Estrela(this, _spriteSheet, _trex, new Vector2(Main.WINDOW_WIDTH, posY));
